...
struct ContentView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 0) {
                // Header
                HStack {
                    Text("\(writingStateManager.wordCount) / \(wordCountGoal)")
                ...
                ScrollView {
                    VStack(spacing: 0) {
                        ...
                        RichTextEditor(text: $writingStateManager.content, font: .systemFont(ofSize: 16))
    ...
    private func updateWordCount() {
        ...
        if wordCountGoal > 0 {
            progress = min(Float(writingStateManager.wordCount) / Float(wordCountGoal), 1.0)
        } else {
            progress = 0
        }
    }
    ...
    private func saveEntry() {
        ...
        let entry = WritingEntry(title: writingStateManager.title.isEmpty ? "New Entry" : writingStateManager.title, text: writingStateManager.content.string, date: Date(), wordCount: writingStateManager.wordCount)
